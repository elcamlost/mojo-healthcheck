...
You can then either instantiate an instance and run the check.
    ...
    my $diagnostic = Mojo::HealthCheck::Diagnostic::Sample->new( id => 'my_id' );
    my $result     = $diagnostic->check;

Or as a class method.

    my $result = Mojo::HealthCheck::Diagnostic::Sample->check();

Set C<runtime> to a truthy value in the params for check and the
time spent checking will be returned in the results.

    my $result = Mojo::HealthCheck::Diagnostic::Sample->check( runtime => 1 );
...
=head2 new
    ...
    my $diagnostic
        = Mojo::HealthCheck::Diagnostic::Sample->new( id => 'my_diagnostic' );
...
sub new {
    ...
    my %params = @params == 1 && (ref $params[0] || '') eq 'HASH' ? %{$params[0]} : @params;
...
sub collapse_single_result {
    ...
    return shift->{collapse_single_result};
}
...
sub check {
    ...
    croak("$class does not implement a 'run' method") unless $class_or_self->can('run');

    my $summarize = exists $params{summarize_result} ? $params{summarize_result} : 1;
...
sub summarize ($self, $result) {
    $self->_set_default_fields($result, qw(id label tags));

    return $self->_summarize($result, $result->{id} // 0);
}

sub _set_default_fields ($self, $target, @fields) {
    if (ref $self) {
        $target->{$_} = $self->{$_} for grep { not exists $target->{$_} } grep { exists $self->{$_} } @fields;
    }
}

sub _summarize ($self, $result, $id) {
    ...
    state $statuses = {map { state $i = 1; $_ => $i++ } @$forward};
    ...
    if (exists $result->{results}) {
        if ((ref $result->{results} || '') eq 'ARRAY') {
            ...
            my $disp = defined $result->{results} ? "invalid results '$result->{results}'" : 'undefined results';
    ...
    for my $i (0 .. $#results) {
        ...
        if (defined(my $s = $r->{status})) {
            ...
            $status = $s if exists $statuses->{$s} and $statuses->{$s} < ($statuses->{$status} // 5);
        }
    }
    ...
    if (exists $result->{timestamp}) {
        ...
        unless (defined $ts and $ts =~ /$rfc3339_timestamp/) {
            my $disp_timestamp = defined $ts ? "invalid timestamp '$ts'" : 'undefined timestamp';
    ...
    $result->{status} = 'UNKNOWN' unless defined $result->{status} and length $result->{status};
...
L<Writing a Mojo::HealthCheck::Diagnostic|writing_a_healthcheck_diagnostic>
